...
@extends('layout.default',['title' => 'Resource Center | Directories '])
@section('content')
@include('header')
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 padding-0">
                    <form class="form-inline mb15" method="get" action="{{url('/')}}/directories" >
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6 padding-0">
                                <input type="text" class="form-control keyword-text product-search h42" placeholder="Find Companies .." name="search" value="{{$search}}">
                            </div>
                            <div class="col-md-3 padding-0">
                                    ...
                                    <div class="card hoverable w100">
                                        <div class="card-image ">
                                            <div class="view overlay hm-white-slight z-depth-1">
                                                <img data-src="{{$company->company_image}}" src="{{url('/').'/public/images/placeholder.png'}}" class="img-responsive  object-fit-contain" alt="">
                                                <div class="mask waves-effect">
                                                    <div class="verticalcenter">
                                                        <a class="btn btn-warning waves-effect  waves-light"
                                                            href="{{ url('/').'/company-detail/'.preg_replace('/[^A-Za-z0-9\-]/', '',strtolower(str_replace(' ','-',$company->company_name))).'/'.$company->uid}}"><i class="fa fa-eye pad-r-5" aria-hidden="true" ></i> View Profile</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="card-content text-left mh145">
                                            <h5>
                                                <a class="bold-n-black" href="{{url('/').'/company-detail/'.preg_replace('/[^A-Za-z0-9\-]/', '',strtolower(str_replace(' ','-',$company->company_name))).'/'.$company->uid}}">@php $company_length=strlen($company->company_name);if($company_length>=25){echo substr($company->company_name,0,25).' ...';} else{echo $company->company_name;}@endphp</a>
                                            </h5>
                                            <i class="fa fa-envelope m-t-1 orange-text" aria-hidden="true"></i><a href="{{url('/').'/company-detail/'.preg_replace('/[^A-Za-z0-9\-]/', '',strtolower(str_replace(' ','-',$company->company_name))).'/'.$company->uid}}" class="company-icon-card" ><span>{{substr($company->email,0,-10)}}</span><span class="asterick" >*****</span></a><br/>
                                            @if(!empty($company->work))
                                                <div class="mtm10">
                                                    <i class="fa fa-phone orange-text" aria-hidden="true"></i>
                                                    <span class="bfh-phone company-icon-card" data-format="(ddd) ddd-" data-number="{{$company->work}}" ></span><span class="asterick" >****</span>
                                                </div>
                                            @endif

                                            <a class="rm"
                                               href="{{ url('/').'/company-detail/'.preg_replace('/[^A-Za-z0-9\-]/', '',strtolower(str_replace(' ','-',$company->company_name))).'/'.$company->uid}}"> View Profile <i class="fa fa-arrow-right pad-r-5" aria-hidden="true" ></i></a>
                                        </div>
                                    </div>
                                    <br>
                                </div>
                            @endforeach
                            <div class="pagination mm1" >@php echo urldecode($companies->links()); @endphp </div>
                        </div>
    ...
    <script>
        function searchDirectory(value){
            ...
            $.post("{{url('/')}}/search_directory",{search:value,_token: '{!! csrf_token() !!}'},function(data){
                $('.directory').hide();
