<template>
    <div class="parentx">
        <vs-sidebar v-hammer:swipe.left="onSwipeLeft" ref="mainSidebar" :parent="parent" :hiddenBackground="clickNotClose" :reduce="reduce" default-index="-1" class="sidebarx main-menu-sidebar items-no-padding" v-model="isSidebarActive" :click-not-close="clickNotClose" :reduce-not-rebound="reduceNotRebound">
            <div :style="{backgroundColor : `${sideBarColor} !important`}" @mouseenter="sidebarMouseEntered" @mouseleave="sidebarMouseLeave">
                <div class="header-sidebar flex items-end justify-between" slot="header">
                    <div class="logo flex items-center">
                        <img draggable="false" :src="logo" alt="logo" class="w-100 mr-4 object-contain" style="height: 50px !important;" v-if="logo">
                ...
                <VuePerfectScrollbar ref="mainSidebarPs" class="scroll-area--main-sidebar pt-2" :settings="settings" @ps-scroll-y="psSectionScroll">
                    <template v-for="(sidebarItem, index) in sidebarItems">
                        ...
                        <template v-else-if="!sidebarItem.header">
                            ...
                            <template>
                                <template>
                                    <!-- if not have submenu and popup flag is true  -->
                                    <vx-sidebar-item @show-popup="showPopup" href="javascript:void(0)" ref="sidebarLink" :key="`sidebarItem-${index}`" v-if="!sidebarItem.submenu && sidebarItem.shouldPopup" :index="index" :icon="sidebarItem.icon" :isDisabled="sidebarItem.isDisabled" :slug="sidebarItem.slug">
                                    ...
                                    <!-- if not have submenu and popup flag is false  -->
                                    <vx-sidebar-item ref="sidebarLink" :key="`sidebarItem-${index}`" v-else-if="!sidebarItem.submenu && !sidebarItem.shouldPopup" :index="index" :to="sidebarItem.slug != 'external' ? sidebarItem.url : ''" :href="sidebarItem.slug == 'external' ? sidebarItem.url : ''" :icon="sidebarItem.icon" :target="sidebarItem.target" :isDisabled="sidebarItem.isDisabled" :slug="sidebarItem.slug">
                                        <span v-show="!sidebarItemsMin" class="truncate">{{ sidebarItem.name }}</span>
                                        <vs-chip class="ml-auto" :color="sidebarItem.tagColor" v-if="sidebarItem.tag && (isMouseEnter || !reduce)">{{ sidebarItem.tag }}</vs-chip>
                                    </vx-sidebar-item>
                                    ...
                                    <template v-else>
                                        <vx-sidebar-group @show-popup="showPopup" ref="sidebarGrp" :key="`group-${index}`" :openHover="openGroupHover" :group="sidebarItem" :groupIndex="index" :open="isGroupActive(sidebarItem)"></vx-sidebar-group>
                                    </template>
                                </template>
                            </template>
                        </template>
