  ...
  <div
    ...
    <ul ref="items" :style="styleItems" class="vs-sidebar-group-items">
      <li v-for="(groupItem, index) in group.submenu" :key="index">
		...
		<vx-sidebar-item @show-popup="callPopup" :index="groupIndex + '.' + index" href="javascript:void(0)" :icon="groupItem.icon" :slug="groupItem.slug" icon-normal :target="groupItem.target" v-if="!groupItem.submenu && groupItem.shouldPopup">
        ...
        <vx-sidebar-item :index="groupIndex + '.' + index" :to="groupItem.url" :icon="groupItem.icon" :slug="groupItem.slug" icon-normal :target="groupItem.target" v-else>
            <span class="truncate">{{ groupItem.name }}</span>
            <vs-chip class="ml-auto" :color="groupItem.tagColor" v-if="groupItem.tag">{{ groupItem.tag }}</vs-chip>
        </vx-sidebar-item>
        </template>
